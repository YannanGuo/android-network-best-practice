...
>WebSocket一种在单个TCP连接上进行全双工通讯的协议。使得客户端和服务器之间的数据交换变得更加简单，允许服务端主动向客户端推送数据。在WebSocket API中，浏览器和服务
器只需要完成一次握手，两者之间就直接可以创建持久性的连接，并进行双向数据传输。

官方网站：http://websocket.org/

WebSocket协议在Node.js上的实现
...
WebSocket是独立的，创建在TCP上的协议，它通过HTTP/1.1协议的101状态吗进行握手。
